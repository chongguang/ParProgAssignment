...
object ParallelCountChangeRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Here comes the moneyThreshold//////////////////////")
    measureParallelCountChange(ParallelCountChange.moneyThreshold(amount))
    println("Here comes the totalCoinsThreshold//////////////////////")
    measureParallelCountChange(ParallelCountChange.totalCoinsThreshold(coins.length))
    println("Here comes the combinedThreshold//////////////////////")
...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    money match {
      ...
      case _ => coins match {
        case Nil => 0
        case x::xs => if(x>money) 0 else countChange(money - x, coins) + countChange(money, coins.tail)
      }
    }
  }
